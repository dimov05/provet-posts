...
in posts/. Source of truth = the frontmatter in each post file.
...
ARCHIVE_DIR = os.path.join(ROOT, "posts")
IMAGES_DIR = os.path.join(ROOT, "images")
OUTPUT_FILE = os.path.join(ROOT, "SCHEDULE.md")
...
def main():
    ...
    with open(OUTPUT_FILE, "w", encoding="utf-8") as f:
        ...
        f.write(f"_Auto-generated by `tools/update-schedule.py` on {TODAY}. "
                f"Edit posts in `posts/`, then run the script (or `/schedule`) to refresh._\n\n")
